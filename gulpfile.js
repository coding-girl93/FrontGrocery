...
var util = {
    ...
    validateVersion: function(config) {
        try {
            ...
            var branchName = execSync('git symbolic-ref HEAD --short', {
                encoding: 'utf8'
            });
...
var modules = {
    ...
    //获取webpack配置
    getWebpackConfig: function(watch) {
        ...
        //决定使用什么模板引擎
        templateLoader = config.template.options.name === 'ejs' ? 'ejs-compiled-loader' + ('?' + JSON.stringify(templateOptions.ejs)) : 'underscore-template-loader' + ('?' + JSON.stringify(templateOptions.underscore));

        //如果非开发环境，则压缩
        !config.dev && plugins.push(new webpack.webpack.optimize.UglifyJsPlugin(config.javascript.options.min));

        //获取webpack用户配置
        var webpackOptions = util.decideRunFunction(config.webpack.options || {}, config);

        //如果非开发环境，则不打开sourcemap
        !config.dev && (webpackOptions.devtool = null);
    ...
    //webpack 任务
    //watch  true : 开启监听模式 false ： 不监听
    webpack: function(watch) {
        var self = this,
        ...
        stream = gulp.src(source)
            ...
            .pipe(webpack(self.getWebpackConfig(watch)))
...
gulp.task('docs', function(cb) {
    var target = path.join('documents', 'v' + config.version),
        source = config.src;
    try {
        execSync('docker -x lib,*.css,*.html,*.less -i ' + source + ' -o ' + target);
        cb();
    } catch (e) {
...
gulp.task('watch', ['default'], function(cb) {
    ...
    gulp.watch(source, function() {
        ...
        // 如果是模拟环境，由于部分css可能是内嵌，则增加html任务
        config.mock && tasks.push('html');
    ...
    //watch template && js
    //嵌套模板，无法被webpack监听
    source = [templateSource, path.join(config.src, '/**/*.js')];
    gulp.watch(source, function() {
        var tasks = [];
        ...
        // 如果是模拟环境，由于部分js可能是内嵌，则增加html任务
        config.mock && tasks.push('html');

        tasks.length && runSequence.apply(null, tasks);
    });
